...
grandchild = IF( AND('child (?y) (?x)','child (?z) (?y)'), THEN('grandchild (?z) (?x)') )

#Despues las reglas 'horizontales'
sibling = IF( AND('parent (?x) (?y)','parent (?x) (?z)',NOT('same-identity (?y) (?z)')), THEN('sibling (?y) (?z)') )
cousin = IF( AND('grandparent (?x) (?y)','grandparent (?x) (?z)',NOT('sibling (?y) (?z)'),NOT('same-identity (?y) (?z)')), THEN('cousin (?y) (?z)') ) #Creo que no es necesario chequear "sibling z y" porque sibling tiene preferencia total sobre esta regla

#Por ultimo las reglas que incluyen el sexo de la persona
brother = IF( AND('sibling (?x) (?y)','male (?x)'), THEN('brother (?x) (?y)') ) #Ver simetria...
...
daughter = IF( AND('child (?x) (?y)','female (?x)'), THEN('daughter (?x) (?y)') )


# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [ same_identity,
                 grandparent,
                 child,
                 grandchild,
                 sibling,
...
WHAT_I_FOUND_INTERESTING = 'Backchain algorithm'
WHAT_I_FOUND_BORING = 'Information about penguins'


def main():
    print forward_chain([transitive_rule], poker_data)
    print forward_chain([transitive_rule],[ 'rock beats scissors','scissors beats paper','paper beats rock' ])
    print forward_chain(family_rules,simpsons_data, verbose=True)
