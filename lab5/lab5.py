...
#print "house1796 votes: ",house_1796_votes
#print "house1796 data: ",house_1796
#print "primer legislador: ",len(house_1796[0]['votes'])
#print "segundo legislador: ",len(house_1796[1]['votes'])
#print "tercer legislador: ",len(house_1796[2]['votes'])
#print "votos totales: ",len(house_1796_votes)
...
#Intento clasificar un diputado que solo voto en la resolucion 80 y se abstuvo en todas las otras. Si lo clasifica como
# Republicano entonces asumo que un Republicano votaria si, en caso contrario asumo que no

Nvotos = len(house_1796_votes)
newspaper = [0] * Nvotos
newspaper[80] = 1
dipuTest = {'votes':newspaper}

#print "Resolucion: ",house_1796_votes[80]
#print "Clasificado: ",boost_1796.classify(dipuTest)
...
def most_misclassified(classifier, n=5):
    ...
    """
    tuplasDato = zip(classifier.data,classifier.data_weights)
    tuplasOrdenadas = sorted(tuplasDato,key=lambda x:x[1],reverse=True)
    res = []
    for i in range(0,n):
        res.append(legislator_info(tuplasOrdenadas[i][0]))
...
#print "Votos del senado: ",senate_votes
nombreABuscar = "Cardin Amdt No. 3930; To modify the sunset provision."
indice = -1
for voto in senate_votes:
    if(voto['name']==nombreABuscar):
        indice = senate_votes.index(voto)
        break
#print "Indice encontrado: ",indice

Nvotos = len(senate_votes)
sunset = [0] * Nvotos
sunset[indice] = 1
senaTest = {'votes':sunset}

#print "Resolucion: ",senate_votes[indice]
#print "Clasificado: ",boost.classify(senaTest)
...
def clasifOptim(dataset,learners,raiz,maxComb):
    global nIntentos
    ...
    learnersTemp = dict(learners)

    maxAcc = 0
    mejorComb = {}
    for clasif in learners:
        actual = dict(raiz)
        actual[clasif] = learners[clasif] #En el for parto siempre de la misma base y agrego uno

        del learnersTemp[clasif]

        print "nIntentos= ",nIntentos
        print "Probando: ", actual.keys()

        (accuracy,brier,auc) = boosted_ensemble(dataset, actual, DATASET_STANDARDS[dataset])
        if(accuracy>maxAcc):
            maxAcc = accuracy
            mejorComb = dict(actual)
        print ("  accuracy: %.3f, brier: %.3f, auc: %.3f" %(accuracy,brier,auc))
        nIntentos += 1

        #Ahora pruebo que pasa si extiendo el actual
        (accExtend,combExtend) = clasifOptim(dataset,learnersTemp,actual,maxComb)
        if (accExtend > maxAcc):
            maxAcc = accExtend
            mejorComb = dict(combExtend)

    return (maxAcc,mejorComb)
...
if __name__ == "__main__":
    ...
    learners2 = {
        "svml": orange.SVMLearner(kernel_type=orange.SVMLearner.Linear, probability=True, svm_type=SVM_TYPE),
        #"maj": orange.MajorityLearner(),  # a useful baseline
        #"dt": orngTree.TreeLearner(sameMajorityPruning=1, mForPruning=2),
        #"knn": orange.kNNLearner(k=10),
        #"svms": orange.SVMLearner(kernel_type=orange.SVMLearner.Sigmoid, probability=True, svm_type=SVM_TYPE),
        "svmp3": orange.SVMLearner(kernel_type=orange.SVMLearner.Polynomial,degree=3,probability=True, svm_type=SVM_TYPE),
        #"svmr": orange.SVMLearner(kernel_type=orange.SVMLearner.RBF,probability=True, svm_type=SVM_TYPE),
        "nb": orange.BayesLearner()
    }

    for clasif in learners2:
        print "lala: ",clasif

    (maxima,comb)=clasifOptim(dataset,learners,{},7)
    print "Intentos: ",nIntentos
    print "Mejor combinacion: ",comb.keys(),"  Acc: ",maxima
    ...
    #       boosted_ensemble(dataset, learners2, DATASET_STANDARDS[dataset]))
...
#classifiers_for_best_ensemble = ['maj', 'dt', 'knn', 'svml', 'svmp3', 'svmr', 'svms', 'nb']

classifiers_for_best_ensemble = ['svml', 'svmp3', 'nb'] #Es dependiente de la arquitectura. No siempre da mas que 74%

## The standard survey questions.
HOW_MANY_HOURS_THIS_PSET_TOOK = 20
WHAT_I_FOUND_INTERESTING ='Everything'
WHAT_I_FOUND_BORING = 'Nothing, as usual'
