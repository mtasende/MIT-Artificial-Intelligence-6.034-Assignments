...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        suma = 0;
        for (clasificador,alpha) in self.classifiers:
            suma = suma + (alpha * clasificador.classify(obj))
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        for i in range(0,len(self.data)):
            if(best_classifier.classify(self.data[i]) == self.standard.classify(self.data[i])):
                self.data_weights[i] = (1.0/2.0)*(self.data_weights[i]/(1-best_error))
            else:
                self.data_weights[i] = (1.0/2.0)*(self.data_weights[i]/best_error)
