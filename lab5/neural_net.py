...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        """
        suma = 0
        for i in range(0,len(self.my_inputs)):
            suma = suma + self.my_weights[i].get_value()*self.my_inputs[i].output()
        output = sigmoid(suma)
        return output
    ...
    def compute_doutdx(self, elem):
        ...
        if(elem in self.get_weights()):
            indice = self.get_weights().index(elem)
            return self.output()*(1-self.output())*(self.my_inputs[indice].output())
        else:
            res = 0
            pesos = self.get_weights()
            entradas = self.get_inputs()
            for i in range(0,len(entradas)):
                peso = pesos[i]
                entrada = entradas[i]
                res = res + peso.get_value() * entrada.dOutdX(elem)
            res = res * self.output()*(1-self.output())
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        #print "Desired:  ",self.my_desired_val
        #print "Neurona de salida:  ",self.my_input
        #print "Valor de salida:  ",self.my_input.output()
        return -(1.0/2.0)*((self.my_desired_val - self.get_input().output())**2)
    ...
    def dOutdX(self, elem):
        ...
        """
        return (self.my_desired_val - self.my_input.output())*self.get_input().dOutdX(elem)
...
def make_neural_net_challenging():
    ...
    wBC = Weight('wBC', random_weight())
    wC = Weight('wC', random_weight())
    C = Neuron('C', [A, B, i1, i0], [wAC, wBC, w1C, wC])
    ...
    D = Neuron('D', [A, B, i2, i0], [wAD, wBD, w2D, wD])
...
def train(network,
    ...
    while iteration < max_iterations:
        ...
        for datum in data:
            ...
            #print "Network.inputs:  ", network.inputs
            #print "Network.neurons:  ", network.neurons
            #print "Network.output:  ", network.output.output()
            #print "Network.performance:  ", network.performance.output()
