...
def alpha_beta_find_board_value(board, depth, eval_fn,alpha,beta,get_next_moves_fn=get_all_next_moves, is_terminal_fn=is_terminal):
    global evaluaciones
    if is_terminal_fn(depth, board):
        evaluaciones +=1
        #print "depth= ", depth, "-- Nodo terminal con valor= ", eval_fn(board)
    ...
    for move, new_board in get_next_moves_fn(board):
        ...
        print "Chequear: ", move, " , tengo (alpha= ", alpha, " y beta= ", beta,") pero ----->   alphaReal= ",alphaReal,"  betaReal= ",betaReal """
        val = -1 * alpha_beta_find_board_value(new_board,depth-1,eval_fn,-beta,-alpha,get_next_moves_fn,is_terminal_fn)
        #print "Valor conseguido de ",move," es: ",val
        if val > alpha:
            alpha = val
        if alpha >= beta:
            break

    #print "depth= ",depth,"-- return= ",alpha
    return alpha
...
def alpha_beta_search(board, depth, eval_fn, get_next_moves_fn=get_all_next_moves, is_terminal_fn=is_terminal):
    ...
    print "AlphaBeta: Decided on column ",best_val[1]," with rating ",best_val[0]
    print "AlphaBeta evals = ",evaluaciones
    return best_val[1]
...
better_player = lambda board: alpha_beta_search(board,depth=4,eval_fn=better_evaluate)
run_game(basic_player, better_player)
